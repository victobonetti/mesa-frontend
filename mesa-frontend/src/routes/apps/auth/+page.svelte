<script lang="ts">
  ...
  const refreshUsers = async() => {
	let reqUsers = await ServiceRequest.call(() => UserService.findUsers());
      let usersErr = handleResponse(reqUsers, throwError);
	  let emails = []
      if (!usersErr) {
        for (let u of reqUsers.result) {
          emails.push(u["email"]);
        }

        let person_data = await ServiceRequest.call(() =>
          PersonService.findPersonByEmail(emails)
        );
        let formatted_users = [];

        for (let user of reqUsers.result) {
          user["full_name"] = null;
  ...
  const getServicesObject = (grants: any[]) => {
    ...
    if (services) {
      for (let service of services) {
        ...
        let service_exibition_name = getExibitionName(service["service_name"]);
        temp_map.push({
...
<div>
  <div class="mb-4 flex">
    <div class="w-1/2">
      <h1 class=" pb-2 text-2xl font-semibold tracking-tight transition-colors">
        Usuários do domínio
      </h1>
      <p class=" select-none text-xs text-left text-neutral-600">
        Gerencie usuários com acesso ao sistema; O email do registro irá obter
        as informações cadastradas no módulo de cadastro de pessoas.
      </p>
    </div>
  ...
  <div class=" w-full border-t pt-6 flex-wrap gap-4 flex">
        ...
        <Table.Root>
          ...
          <Table.Body>
            {#each users as u}
              <Table.Row>
                {#if u["full_name"]}
                  <Table.Cell class="font-medium">{u["full_name"]}</Table.Cell>
                {:else}
                ...
                <Table.Cell class="text-right"
                  ><DropdownMenu.Root>
                    ...
                    <DropdownMenu.Content>
                      <DropdownMenu.Group>
                        <DropdownMenu.Label>Editar</DropdownMenu.Label>
                        ...
                        >
                        <DropdownMenu.Item
                          on:click={() => triggerDeleteUser(u["id"])}
    ...
    {#if grantsWindowActive}
      ...
      >
        <h2 class=" select-none mb-1 font-semibold">Permissões do usuário</h2>
        <p
          class=" select-none text-xs text-left text-neutral-600 border-b pb-1"
        >
          É permitida apenas uma permissão por serviço; Clique na checkbox para
          adicionar ou remover a permissão.
        </p>
        <div class="p-2">
          {#if isSuperAdmin}
              ...
              <div class="flex flex-col mb-2 mt-1">
                <div class="flex">
                  <input
                    ...
                    disabled
                  />
                  <label class="text-xs ml-0.5 flex opacity-80" for="superadmin"
                    >Habilitado
          ...
          <div>
            {#if !isSuperAdmin}
              <h2 class=" select-none text-neutral-800 font-medium">
                Serviços
              </h2>
              {#if grant_opts}
                {#each grant_opts as g}
                  ...
                  <div class="flex mt-1">
                    <div>
                      ...
                      />
                      <label class="text-xs ml-0.5" for="read">Leitura</label>
                    </div>
                    <div class="ml-4">
                      <input
                        id="write"
                      ...
                      />
                      <label class="text-xs ml-0.5" for="write">Escrita</label>
                    </div>
                    <div class="ml-4">
                      <input
                        id="admin"
                      ...
                      />
                      <label class="text-xs ml-0.5" for="admin">Gerenciar</label
                      >
                    </div>
                  </div>
                {/each}
